...
namespace sdds {
    ...
    AidMan::AidMan(const char* filename) : fileName(nullptr), mainMenu("List Items\tAdd Item\tRemove Item\tUpdate Quantity\tSort\tShip Items\tNew/Open Aid Database"), numProducts(0) {
        if (filename != nullptr) {
    ...
    void AidMan::add() {
        ...
        products[numProducts] = choice == 1 ? new Perishable : new Item;
        ...
        if (choice == 1 || choice == 2) {
            int sku = products[numProducts]->readSku(cin);
            if (search(sku) != -1) {
